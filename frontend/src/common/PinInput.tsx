...
export function PinInput({ onComplete, onClear, length = 4 }: Readonly<PinInputProps>) {
  ...
  const shouldFocusPreviousInput = (index: number, e: KeyboardEvent<HTMLInputElement>) => {
    return e.key === "Backspace" && !values[index] && index > 0;
  }

  const handleKeyDown = (index: number, e: KeyboardEvent<HTMLInputElement>) => {
    if (shouldFocusPreviousInput(index, e)) {
      focusInput(index - 1)
    }
  }
